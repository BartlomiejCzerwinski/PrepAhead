...
## Deferred (recorded during Phase 1 implementation)

- **PRO daily-cap (10/day UTC) and soft-threshold (100) are not enforced in code** — `generationSoftThreshold: 100` is a type literal only (`src/lib/plan/limits.ts:12`) and the "max 10/day" is display text only (`get-usage-summary.ts:28-29`); grep of `supabase/` finds no day-bucketing logic. This was explicitly deferred to roadmap **S-05** in `context/changes/supabase-data-schema/plan.md:61,79,347`. Phase-1 tests therefore assert **only** the enforced hard cap (300 PRO / 1 FREE) and Check caps (5 / 500); `src/lib/plan/get-usage-summary.test.ts` includes a `generation_count: 100` PRO case proving the soft threshold does **not** block (current intended-for-now behavior). No assertion is written against the non-existent daily logic. When S-05 implements the daily cap, add its coverage in that change.

-- Server-side Check metering via service role (auth.uid() is null for service_role).
-- Apply on hosted Supabase (kweteoumoxhtwdtyhmks) before relying on Check decrements.
...
comment on function public.increment_check_usage_for_user(uuid) is
  'Atomically increment check_count for the given user in their current rolling period. Service-role only.';

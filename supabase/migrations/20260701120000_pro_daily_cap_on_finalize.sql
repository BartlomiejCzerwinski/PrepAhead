-- S-05 Phase 4: PRO fair-use daily cap (10/day UTC when period count >= 100) in finalize path.
...
begin
  ...
  if v_job.usage_incremented_at is null then
    ...
    select coalesce(up.generation_count, 0)
    into v_period_generation
...
comment on function public.finalize_generation_job(uuid, text, text, jsonb) is
  'Idempotently persists final practice-set content, increments generation usage once when under plan cap (including PRO daily fair-use), and marks the durable job succeeded.';
